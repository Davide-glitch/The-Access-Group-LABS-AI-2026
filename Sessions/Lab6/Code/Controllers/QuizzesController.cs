...
public class QuizzesController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<ActionResult<IEnumerable<Quiz>>> List([FromQuery] string? tag) =>
        Ok(await _repo.AllAsync(tag));
    ...
    public async Task<ActionResult<Quiz>> AddTag(Guid id, CreateTagDto dto)
    {
        var quiz = await _repo.AddTagAsync(id, dto.Name);
        return quiz is null ? NotFound() : Ok(quiz);
    }
}