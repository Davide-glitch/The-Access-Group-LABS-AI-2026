...
function QuizRunner({ quiz, onExit }: QuizRunnerProps) {
  ...
  // A quiz with no questions has nothing to step through (server quizzes can
  // be empty). Bail out rather than indexing into an empty array.
  if (quiz.questions.length === 0) {
    return (
      <div className="runner">
        <h2>{quiz.title}</h2>
        <p className="status-message">This quiz has no questions to run.</p>
